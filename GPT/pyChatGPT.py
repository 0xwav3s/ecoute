...
chatgpt_login_btn = (By.XPATH, '//*[@id="__next"]/div[1]/div[1]/div/div/div/div/nav/div[2]/div[2]/button[2]')
chatgpt_login_h1 = (By.XPATH, '//h1[text()="Welcome back"]')
chatgpt_logged_h1 = (By.XPATH, '//*[text()="New chat"]')
...
class ChatGPT:
    ...
    def send_message(self, message: str, stream: bool = False) -> dict:
        ...
        firstLen = len(self.driver.find_elements(*chatgpt_big_response))
        ...
        while True:
            lastLen = len(self.driver.find_elements(*chatgpt_big_response))
            time.sleep(0.1)
            if lastLen > firstLen:
                break
    ...
    def checkStream(self):
        ...
        # Define variables to store the last request with response
        last_request = None

        # Define a handler to intercept network responses
        def response_interceptor(event):
            response = event['params']
            if last_request and response['requestId'] == last_request['requestId']:
                # Extract status code from the intercepted response
                status_code = response['response']['status']
                print(f"Last request to '{endpoint}' has status code: {status_code}")
                # You can store or use the status code as needed
        ...
        # Bind response interceptor to 'Network.responseReceived' event
        self.driver.on('Network.responseReceived', response_interceptor)

        # Define a handler to intercept network requests
        def request_interceptor(event):
            nonlocal last_request
            request = event['params']
            if request['request']['url'].endswith(endpoint):
                last_request = request

        # Bind request interceptor to 'Network.requestIntercepted' event
        self.driver.on('Network.requestIntercepted', request_interceptor)
